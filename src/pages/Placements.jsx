...
const PlacementRecord = ({ placement }) => {
  return (
    <tr className="hover:bg-gray-100">
      ...
      <td className="py-3 px-6 border border-gray-300">{placement.student_pin}</td>
      <td className="py-3 px-6 border border-gray-300">{placement.depo_code?.toUpperCase()}</td>
      ...
      <td className="py-3 px-6 border border-gray-300">{placement.package} LPA</td>
...
function Placements() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const response = await fetch(import.meta.env.VITE_BACKEND + "placements/all");
      const data = await response.json();
      const sorted = data.sort((a, b) => parseFloat(b.package) - parseFloat(a.package));
            ...
            <table className="w-full border-collapse border border-gray-300">
              <thead>
                <tr className="bg-red-700 text-white text-left">
                  <th className="py-3 px-6 border border-gray-300">Student Name</th>
